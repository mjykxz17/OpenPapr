    ...
    <>
      {groups.map((g) => (
        <div key={g.label} className="mb-3">
          <div className="text-[13px] font-semibold uppercase tracking-[0.06em] text-ink-2">{g.label}</div>
          ...
          <li key={todo.id} className="group flex items-center justify-between gap-4 py-3">
            <div className="min-w-0">
              <div className="flex items-baseline gap-2">
                {overdue && (
                  <span className="shrink-0 text-[13px] font-medium text-danger">Overdue</span>
                )}
                <span
                  className={`truncate text-[15px] font-medium ${overdue ? "text-danger" : "text-ink"}`}
            ...
            <div className="flex shrink-0 items-center gap-2">
              {badge != null && (
                <span className="rounded-md border border-line px-2 py-0.5 text-[13px] tabular-nums text-ink-2">{badge}%</span>
              )}
